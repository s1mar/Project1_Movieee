...
class AndroidIntentTests(unittest.TestCase):
    def test_resolve_deeplink_to_www(self):
        from seatwatch.notify import resolve_deeplink
        url = resolve_deeplink(
            "https://apis.cineplex.com/prod/cpx/theatrical/deeplink"
            "?s=404521&a=0000000001&l=9406&m=the-odyssey&ss=False")
        self.assertIn("www.cineplex.com/Movie/the-odyssey", url)
        self.assertIn("VistaSessionId=404521", url)
        self.assertIn("LocationId=9406", url)

    def test_resolve_leaves_non_deeplink_untouched(self):
        from seatwatch.notify import resolve_deeplink
